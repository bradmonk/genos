...
% SORT VARIANTS BY EITHER CHRPOS
% [~,j]  = sort(LOCI.CHRPOS);
% LOCI  = LOCI(j,:);
% CASE  = CASE(j);
% CTRL  = CTRL(j);
...
PASS = ((LOCI.OKCASEALT > 20) | (LOCI.OKCTRLALT > 20)) &...
       ((LOCI.OKCASEREF > 20) | (LOCI.OKCTRLREF > 20)) &...
       ((LOCI.OKCASEALT >  8) & (LOCI.OKCTRLALT >  8)) &...
       ((LOCI.OKCASEREF >  8) & (LOCI.OKCTRLREF >  8));
LOCI  = LOCI(PASS,:);
CASE  = CASE(PASS);
CTRL  = CTRL(PASS);
USNP  = USNP(PASS);
...
% SORT VARIANTS BY OKFISHOR
[~,j]  = sort(LOCI.OKFISHOR,'descend');
LOCI  = LOCI(j,:);
CASE  = CASE(j);
CTRL  = CTRL(j);
USNP  = USNP(j);
...
PTOP.CHRPOS = uint64(PTOP.CHRPOS);
...
fprintf('\n\n | GENE LOOP: %.0f  \n | SUBSET LOOP: %.0f \n\n',kk,ij)
    ...
    [PVXt, VXt, DVXt, DXt, Yt, YDt] = mkdx(XLOCI,XCASE,XCTRL,XUSNP,TRPHE,[-1 -0 2 5]);
    [PVXh, VXh, DVXh, DXh, Yh, YDh] = mkdx(XLOCI,XCASE,XCTRL,XUSNP,TEPHE,[-1 -0 2 5]);



    % ESTABLISH A RARE VARIANT ONLY FOUND IN CASES
    %======================================================================
    %---TRAINING-----
    % MAKE 20  CASES  alt/alt  AT TRAINING_MATRIX(:,1)
    % MAKE 20  CASES  ref/alt  AT TRAINING_MATRIX(:,1)
    % MAKE ALL CTRLS  ref/ref  AT TRAINING_MATRIX(:,1)
    %---HOLDOUT-----
    % MAKE ALL CASES  alt/alt  AT  HOLDOUT_MATRIX(:,1)
    % MAKE ALL CTRLS  alt/alt  AT  HOLDOUT_MATRIX(:,1)
    %----------------------------------------------------------------------
    VXt(:,1)   = -1;
    VXh(:,1)   = -1;
    PVXt(:,10) = -1;
    ...
    PVXt(iCAt(1:10)  ,10) = 2;
    %PVXt(iCAt(11:20) ,10) = 2;
    PVXh(iCAh(1:10)  ,10) = 2;
    %PVXh(iCAh(11:20) ,10) = 2;
...
%----------------------------------------------------------------------
    ...
    % ARTIFICIALLY MAKE HOLDOUT PARTICIPANTS MATCH TRAINING CONDITIONS
    %----------------------------------------------------------------------
%     VXt(:,1)   = -1;
%     VXh(:,1)   = -1;
%     PVXt(:,10) = -1;
...
%     PVXh(iCAh(11:20) ,10) = 2;
    ...
    [CONFU_HO, PERF_HO, AREA_HO] = confusionmx(Yh', ACTnh,1);
