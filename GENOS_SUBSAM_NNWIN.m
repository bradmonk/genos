...
%%
    ...
    TL = dummyvar(categorical(PVTR(:,2)==1))'; 
    ...
    HIPO_TR_CASE = nanmean((ISHI_TR_CASE) );
    HIPO_TR_CTRL = nanmean((ISHI_TR_CTRL) );
    HIPO_HO_CASE = nanmean((ISHI_HO_CASE) );
    HIPO_HO_CTRL = nanmean((ISHI_HO_CTRL) );
    ...
    % ALL: SPLIT PREDICTIONS FOR CASES AND CONTROLS
    ACT_TR_CASE_LR = TRfx_LR(TL==1) .*  1;
    ACT_TR_CTRL_LR = TRfx_LR(TL==0) .* -1;
    ACT_HO_CASE_LR = HOfx_LR(HL==1) .*  1;
    ACT_HO_CTRL_LR = HOfx_LR(HL==0) .* -1;
    ...
    % ALL/HIGH: HAS HIGH ACTIVATION?
    ISHI_TR_CASE_LR = abs(ACT_TR_CASE_LR) > .25;
    ISHI_TR_CTRL_LR = abs(ACT_TR_CTRL_LR) > .25;
    ISHI_HO_CASE_LR = abs(ACT_HO_CASE_LR) > .25;
    ISHI_HO_CTRL_LR = abs(ACT_HO_CTRL_LR) > .25;
    ...
    % HIGH: PROPORTION POPULATION
    HIPO_TR_CASE_LR = mean((ISHI_TR_CASE_LR) );
    HIPO_TR_CTRL_LR = mean((ISHI_TR_CTRL_LR) );
    HIPO_HO_CASE_LR = mean((ISHI_HO_CASE_LR) );
    HIPO_HO_CTRL_LR = mean((ISHI_HO_CTRL_LR) );
