...
P.Output_Path = [P.data P.f 'modSNP' P.f 'RUN7_APOE' P.f 'MAT'];
P.TargetSNP_Path = [P.data P.f 'CSVSNP' P.f 'NA.csv'];
...
P.APOExx_path = [P.data P.f 'SUBSETS' P.f 'APOE_22_23_24_33_34_44' P.f...
                'APOE_22_23_24_33_34_44_FISHP'];
...
P.CHRPOSe2 = 190045412079;
P.CHRPOSe4 = 190045411941;

APOEi = sum(LOCI.CHRPOS == ([P.CHRPOSe2; P.CHRPOSe4]') ,2)>0;

ADSP.SNP = ADSP.LOCI(APOEi,:);
...
%----------------------------------------
APOE_SUBTYPES = 6;
LOOPDATA.CONFU_TR = nan(4,4,P.Nloops,APOE_SUBTYPES);
LOOPDATA.CONFU_HO = nan(4,4,P.Nloops,APOE_SUBTYPES);
LOOPDATA.PERF_TR  = nan(P.Nloops,6,APOE_SUBTYPES);
LOOPDATA.PERF_HO  = nan(P.Nloops,6,APOE_SUBTYPES);
LOOPDATA.AREA_TR  = nan(50,5,P.Nloops,APOE_SUBTYPES);
LOOPDATA.AREA_HO  = nan(50,5,P.Nloops,APOE_SUBTYPES);
...
% [CONFU_TR, PERF_TR, PCUT_TR, AREA_TR] = nnperform(Yt', DXt', PVXt, net, 'cut',.5,'plt',1);



%======================================================================
%% PERFORM modSNP ON EACH APOE SUBTYPE
%======================================================================
% NNTRa2refref = nan(size(TRPHE,1),P.Nsnps);
% NNTRa2refalt = nan(size(TRPHE,1),P.Nsnps);
% NNTRa2altalt = nan(size(TRPHE,1),P.Nsnps);
% NNTEa4refref = nan(size(TEPHE,1),P.Nsnps);
% NNTEa4refalt = nan(size(TEPHE,1),P.Nsnps);
% NNTEa4altalt = nan(size(TEPHE,1),P.Nsnps);

showPlots = [0 0]>1;


% GET NETWORK CV OUTPUT FOR APOE-22
%-------------------------------------------
eSub = 22;
[LOOPDATA] = modAPOE(LOOPDATA,XLOCI,XCASE,XCTRL,XUSNP,TRPHE,TEPHE,INFO.NNwts,net,eSub,ij,showPlots);
...
size(LOOPDATA.AREA_TR)
size(LOOPDATA.AREA_HO)


TRCV = LOOPDATA.AREA_TR;
HOCV = LOOPDATA.AREA_HO;
...
APOE22 = mean(mean(APOE22,3),2);
APOE23 = mean(mean(APOE23,3),2);
APOE24 = mean(mean(APOE24,3),2);
APOE33 = mean(mean(APOE33,3),2);
APOE34 = mean(mean(APOE34,3),2);
APOE44 = mean(mean(APOE44,3),2);
...
close all
h22 = area(APOE22,'XData',XAX,'LineWidth',2,'FaceAlpha',.5,'FaceColor',fc(1,:));

hold on

h23 = area(APOE23,'XData',XAX,'LineWidth',2,'FaceAlpha',.5,'FaceColor',fc(2,:));

hold on

h24 = area(APOE24,'XData',XAX,'LineWidth',2,'FaceAlpha',.5,'FaceColor',fc(3,:));

hold on

h33 = area(APOE33,'XData',XAX,'LineWidth',2,'FaceAlpha',.5,'FaceColor',fc(4,:));

hold on

h34 = area(APOE34,'XData',XAX,'LineWidth',2,'FaceAlpha',.5,'FaceColor',fc(5,:));

hold on

h44 = area(APOE44,'XData',XAX,'LineWidth',2,'FaceAlpha',.5,'FaceColor',fc(6,:));

hold on



A = [APOE22 APOE23 APOE24 APOE33 APOE34 APOE44];
...
histogram('BinEdges',AXE,'BinCounts',APOE33,'FaceColor',[.4 .4 .4])
hold on
histogram('BinEdges',AXE,'BinCounts',APOE24,'FaceColor',cm(2,:))
hold on
histogram('BinEdges',AXE,'BinCounts',APOE34,'FaceColor',cm(3,:))
hold on
histogram('BinEdges',AXE,'BinCounts',APOE23,'FaceColor',cm(4,:))
hold on
histogram('BinEdges',AXE,'BinCounts',APOE44,'FaceColor',[.1 .9 .4])
hold on
histogram('BinEdges',AXE,'BinCounts',APOE22,'FaceColor',cm(6,:))
