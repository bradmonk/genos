...
19	ADGC	Relig Ordr  ROS     0	351     0       154     197     351     4x	56	154	-43     19 GOOD
20	CHARGE	RotterdamS  RS      0	1089	0       276     813     1089	25	75	276	-537	20 GOOD
21	ADGC	Texas AD S  TARC	0	14x     0       132     12      14x     92	8	12	120     21 MEH
...
APOE4xTF = PHEN.APOE >= 34; 
    disp(sum(APOE4xTF))

APOE4xTAB = PHEN(APOE4xTF,:);

APOE4xTAB = sortrows(APOE4xTAB,{'AD','GOODCOH'},{'ascend','descend'});

APOE4xCTRL = APOE4xTAB((APOE4xTAB.AD == 0) & (APOE4xTAB.GOODCOH == 1),:);



clearvars -except ADSP LOCI CASE CTRL PHEN USNP...
PHETRCASE PHETRCTRL PHETECASE PHETECTRL APOE4xCTRL
clc; disp(APOE4xCTRL)



%---- REMOVE APOE4x CTRLs FROM TRAIN/TEST TABLES 
%---- THEN ADD ALL APOE4x CTRLs TO TEST TABLE


[Ai,Bi] = ismember(PHETRCTRL.SRR,APOE4xCTRL.SRR);
A = PHETRCTRL(Ai,:);
PHETRCTRL(Ai,:) = [];

[Ai,Bi] = ismember(PHETECTRL.SRR,APOE4xCTRL.SRR);
B = PHETECTRL(Ai,:);
PHETECTRL(Ai,:) = [];


PHETECTRL = [APOE4xCTRL; PHETECTRL];
...
% THE YOUNGEST APOE4x CTRLS ARE 74, 79, 80, 80, 81...
...
%##########################################################################
%
%%                  EVALUATE THE APOE4x INDIVIDUALS
%
%##########################################################################


% Check if people in PHEN file reported as e4x are actually e4x
FRX = FULLTRX(2:end,:)';
A = FRX(4,:)>=34;
FRX = FRX(:,A);
FRX_NOT4x = (FRX(4,:) <= 33) | (FRX(5,:) < 1);
disp(sum(  FRX_NOT4x  ))
...
% SEPARATE OUT APOE4x CASES AND CTRLS.
FRX4xCA = FRX(:,FRX(2,:)==1);
FRX4xCO = FRX(:,FRX(2,:)==0);

TRPHE4xCA = TRPHE4x(TRPHE4x.AD==1,:);
TRPHE4xCO = TRPHE4x(TRPHE4x.AD==0,:);
...
TEPHE4xCA = TEPHE4x(TEPHE4x.AD==1,:);
TEPHE4xCO = TEPHE4x(TEPHE4x.AD==0,:);
...
%% Evaluate regression classifier performance on E4x individuals

TE4xCA = FEX4xCA(5:end,:)';
TE4xCO = FEX4xCO(5:end,:)';

TE4xCA_x = [ones(size(TE4xCA,1),1) TE4xCA];
TE4xCO_x = [ones(size(TE4xCO,1),1) TE4xCO];


TE4xCA_y =  ones(size(TE4xCA,1),1);
TE4xCO_y = zeros(size(TE4xCO,1),1);


TE4xCA_Act = TE4xCA_x .* t';
TE4xCO_Act = TE4xCO_x .* t';


TE4xCA_fx = sum( TE4xCA_Act ,2);
TE4xCO_fx = sum( TE4xCO_Act ,2);


TE4xCA_yp = round(TE4xCA_fx);
TE4xCO_yp = round(TE4xCO_fx);


TE4xCA_mu = mean(TE4xCA_yp == TE4xCA_y);
TE4xCO_mu = mean(TE4xCO_yp == TE4xCO_y);
...
TE4xCO TE4xCA TE4xCA_Act TE4xCO_Act TE4xCA_fx TE4xCO_fx...
TE4xCA_yp TE4xCO_yp TE4xCA_mu TE4xCO_mu





%% DISPLAY AND PLOT PERFORMANCE OF E4x CONTROLS
close all;
...
%% DISPLAY AND PLOT PERFORMANCE OF E4x CASES VS E4x CONTROLS
close all;
fh1 = figure('Units','normalized','OuterPosition',[.01 .06 .9 .88],'Color','w','MenuBar','none');
ax1 = axes('Position',[.05 .53 .9 .42],'Color','none'); box off; hold on;
ax2 = axes('Position',[.05 .05 .9 .42],'Color','none'); box off; hold on;
...
axes(ax1)
bar((TE4xCO_Act(:,ns:ne))')
xlim([-2 ne+2])

axes(ax2)
bar((TE4xCA_Act(:,ns:ne))')
xlim([-2 ne+2])
...
%% DETERMINE WHAT IS UNIQUE ABOUT LOW-CONF APOE4x INDIVIDUALS
