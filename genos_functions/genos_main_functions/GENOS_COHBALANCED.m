...
N CONSORT	STUDY_NAME  STUDY   FAM PEOPLE  ENR     CASE	CTRL	TOT    %CA %CO  EQL	CULL	NOTES
...
% REMOVE ANY COHORT WITH LESS THAN 10 CASES AND CONTROLS

c = minCACO >= 10;
[i,~] = find(c);
j = ismember(COHSET.COHORTNUM,i);
COHSET = COHSET(j,:); 
...
% RECOUNT NUMBER OF CASE AND CTRL IN EACH COHORT
...
[(1:numel(unique(TRL_COH)))' unique(TRL_COH) unique(TEL_COH)]
...
[TPR,FPR,THR] = roc(TRC,simTRAIN);
% [TPR,FPR,THR] = roc(TEC,simTEST);
...
%--- Add Legend
a=repmat('Cohort: ', numel(unique(TRL_COH)), 1);
b=string(unique(TRL_COH));
c=horzcat(char(a),char(b));
legend(ax1,c)
...
clearvars -except ADSP LOCI CASE CTRL PHEN USNP TRCASE TRCTRL TECASE TECTRL...
VLOCI VCASE VCTRL VUSNP VTRCASE VTRCTRL VTECASE VTECTRL TRPHE TEPHE...
FULLTRX FULLTEX TRX TRL TEX TEL net netz TRC TEC TRL_COH TEL_COH
