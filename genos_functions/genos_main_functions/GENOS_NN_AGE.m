...
%% REMOVE INSTANCES WHERE ALT >> REF ALLELE


F = (VLOCI.CASEALT > (VLOCI.CASEREF.*1.5)) &...
    (VLOCI.CTRLALT > (VLOCI.CTRLREF.*1.5));
...
[NNMX, vMX] = makeAgeNN(VLOCI,VCASE,VCTRL,VUSNP,TRPHE,0);
disp(NNMX(1:10,1:10))
...
VLOCI VCASE VCTRL VUSNP VPHE NNMX TRPHE TEPHE TRAINX TRAINL TESTX TESTL
...
%---------------------------
scatter(GESjit, AGEjit, 100, COLORBY, '.');
%---------------------------




% colormap(nthroot(jet./1.03+.025,2.5))
% map =  [.0 .7 .1;  .9 .5 .1;  .9 .1 .6;  .1 .5 .6];
% map =  [.0 .7 .1;  .9 .1 .6;  .0 .8 .9];
% colormap(map)
...
cb1 = colorbar;
cb1.Ticks = 1:numel(unique(COLORBY));
% cbt = linspace(1,numel(unique(COLORBY)),numel(unique(COLORBY))*2-1);
% cb1.Ticks = cbt(2:2:end);
% cb1.TickLabels = unique(COLORBY);
% cb1.FontSize = 14;
% cb1.Label.String = "\fontsize{22} PHENOTYPE";
