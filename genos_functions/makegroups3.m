function [TRCASE,TRCTRL,TECASE,TECTRL] = makegroups3(PHEN)
...
% DETERMINE GOOD COHORTS
%-------------------------------------------------------------
B = PHEN(~isnan(PHEN.BRAAK),:);
B(:,11:17)=[];
[PHEmu] = grpstats(B,{'COHORTNUM','BRAAK'},...
    {'mean','predci','gname'},'Alpha',0.1,'DataVars',{'AD','SEX','AGE','TOTvars'})
...
Y = PHEmu.mean_AGE;
Yci = PHEmu.predci_AGE;

COHS = PHEmu.COHORTNUM;
UNIQCOHS = unique(COHS);
NCOHS = numel(UNIQCOHS);
FCOHS = findgroups(PHEmu.COHORTNUM);

ph1 = bar(1:size(PHEmu,1) , Y,'FaceColor','flat'); 
cb = colormap(lines(NCOHS));
ph1.CData = cb(FCOHS,:);
ax1.XTick = 1:numel(PHEmu.COHORTNUM);
ax1.XTickLabel = PHEmu.COHORTNUM;
ax1.YLim = [50 100];
ylabel('Age Mean')
cb1 = colorbar;
...
ph1 = bar(1:size(PHEmu,1) , Y,'FaceColor','flat'); 
    ...
    leg1 = legend(string(UNIQCOHS));
    leg1.AutoUpdate = 'off';
    leg1.String = string(UNIQCOHS);
...
unique(PHEN.COHORTNUM(find(PHEN.GOODCOH)))
...
close all; figure;
histogram(PHETRCASE.TOTvars); hold on;
histogram(PHETRCTRL.TOTvars); hold on;
histogram(PHETECASE.TOTvars); hold on;
histogram(PHETECTRL.TOTvars); hold on;



% [TRCAi,TRCAj,TRCAk] = unique(PHETRCASE.COHORTNUM);
% [TRCOi,TRCOj,TRCOk] = unique(PHETRCASE.COHORTNUM);
% [TECAi,TECAj,TECAk] = unique(PHETRCASE.COHORTNUM);
% [TECOi,TECOj,TECOk] = unique(PHETRCASE.COHORTNUM);
% 
% gN = splitapply(@numel,PHETRCASE.COHORTNUM,TRCAi);
% disp([gi gN])
% close all; figure;
% subplot(2,2,1); histogram(TRCAk); hold on;
% subplot(2,2,2); histogram(TRCOk); hold on;
% subplot(2,2,3); histogram(TECAk); hold on;
% subplot(2,2,4); histogram(TECOk); hold on;
...
% useCohorts = [1 2 6 7 9 10 11 12 13 19 20 23 24];
% useCohorts = [1 2 6 7 9 10 11 12 13 19 20];
% COHSET = PHEN(ismember(PHEN.COHORTNUM,useCohorts),:);
% size(COHSET) %9541  TOTAL PEOPLE



% GET SET OF GOOD COHORTS OR ANYONE WITH BRAAK SCORE
%-------------------------------------------------------------
...
% LITERALLY JUST UNDO ALL THAT AND USE COHORTS 1:3 FOR TRAINING
% AND THE REST FOR VALIDATION AND TESTING
%-------------------------------------------------------------

% [gi, gj, gk] = findgroups(PHETRCASE.GOODCOH);
% [gi, gj, gk] = unique(PHETRCASE.GOODCOH);
