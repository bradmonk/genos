function [varargout] = nnconfplot(gMX,NV,PCTCORR,NNP,NETZ,TRAINX,TRAINL,TESTX,TESTL)
%%
...
m=m+1;
    ...
    [TRX,TRL,TEX,TEL,TRi,VAi,TEi] = validmx(...
        gMX{i}.PVTRX(:,10:(NV(q(j))+9)), gMX{g}.LPTR,...
        gMX{i}.PVTEX(:,10:(NV(q(j))+9)), gMX{g}.LPTE,...
        .4, 500);
    ...
    TRLABEL = vec2ind(TRAINL);
    TELABEL = vec2ind(TESTL);
    ...
    TEpop = TEmu;
        ...
        TRvec = NETZ{q(j)}(TRAINX);
        TEvec = NETZ{q(j)}(TESTX);
