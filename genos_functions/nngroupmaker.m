function [TRCASE,TRCTRL,TECASE,TECTRL] = nngroupmaker(PHEN,varargin)
...
% ID CONSORT	STUDY_NAME  STUDY   FAM PEOPLE  ENR     CASE	CTRL	TOT    %CA %CO  EQL	CULL	GOOD   BRAAK
% 01     DGC    Adult Chng  ACT     0	1277	0       323     945     1268	25	75	323	-622	01 3    1
% 02    ADGC	AD Centers  ADC     0	3263	0       2438	817     3255	75	25	817	1621	02 3    0
% 03  CHARGE	Athrosclro  ARIC	0	57      0       39      18      57      68	32	18	21      03 1    0
% 04  CHARGE	Aus Stroke	ASPS	0	126     0       121     5       126     96	4	5	116     04 0    0
% 05    ADGC	Chic Aging  CHAP	0	231     0       27      204     231     12	88	27	-177	05 2    0
% 06  CHARGE	CardioHlth  CHS     0	840     0       250     583     833     30	70	250	-333	06 3    1
% 07    ADGC	Hispanic S  CUHS	361	0       331     160     171     331     48	52	160	-11     07 2    0
% 08  CHARGE	Erasmus Er  ERF     30	45      0       45      0       45      100	0	0	45      08 0    0
% 09  CHARGE	Framingham  FHS     0	581     0       157     424     581     27	73	157	-267	09 3    1
% 10	ADGC	Gene Diffs  GDF     0	207     0       111     96      207     54	46	96	15      10 3    1
% 11	ADGC	NIA - LOAD  LOAD	80  113     363     367     109     476     77	23	109	258     11 3    1
% 12	ADGC	Aging Proj  MAP     0	415     0       138     277     415     33	67	138	-139	12 3    1
% 13	ADGC	Mayo Clini  MAYO	0	349     0       250     99      349     72	28	99	151     13 3    1
% 14	ADGC	Miami Univ  MIA     61	181     19      186     14      200     93	7	14	172     14 1    1
% 15	ADGC	AD Genetic  MIR     0	284     47      316     15      331     95	5	15	301     15 1    0
% 16	ADGC	Mayo cl PD  MPD     0	20      0       0       20      20      0	100	0	-20     16 0    1
% 17	ADGC	NationC AD  NCRD	18	108     52      160     0       160     100	0	0	160     17 0    1
% 18	ADGC	Wash Unive  RAS     0	0       46      46      0       46      100	0	0	46      18 0    1
% 19	ADGC	Relig Ordr  ROS     0	351     0       154     197     351     44	56	154	-43     19 3    1
% 20  CHARGE	RotterdamS  RS      0	1089	0       276     813     1089	25	75	276	-537	20 3    0
% 21	ADGC	Texas AD S  TARC	0	144     0       132     12      144     92	8	12	120     21 1    0
% 22	ADGC	Un Toronto  TOR     0	0       9       9       0       9       100	0	0	9       22 0    0
% 23	ADGC	Vanderbilt  VAN     6	235     1       210     26      236     89	11	26	184     23 1    1
% 24	ADGC	WashNY Age  WCAP	0	147     3       34      116     150     23	77	34	-82     24 2    0
% 25	ADGC 	Univ Penns  UPN     40 	0       3       0       0       0       0	0	0	0       25 0    0


(3)    GOOD = [1 2 6 9 10 11 12 13 19 20];
(2)    OKAY = [5 7 24]
(1)    MEH  = [3 14 15 21 23];
(0)    BAD  = [4 7 8 16 17 18 22 25];
...
if nargin > 1
    ...
    COHS = varargin{1};
    COHSET = PHEN(sum(PHEN.COHORTNUM == COHS , 2)>0,:);
...
% REMOVE ANY COHORT WITH LESS THAN 7 CASES AND 7 CTRLS
%-------------------------------------------------------------
cohNums(minCACO < 7) = [];
nCACO(minCACO < 7,:) = [];
minCACO(minCACO < 7) = [];
...
szCA = round( size(PHETECASE,1) / 2.5 );
szCO = round( size(PHETECTRL,1) / 2.5 );
...
%------------------------------------------
PHETRCTRL(PHETRCTRL.AGE < 73 , :) = [];
PHETECTRL(PHETECTRL.AGE < 73 , :) = [];
...
clearvars -except ADSP LOCI CASE CTRL PHEN USNP...
COHSET PHETRCASE PHETRCTRL PHETECASE PHETECTRL PHECASE PHECTRL
