function [PVMX, PVMXi, PVL] = badfeaturematrix(ADMX,CASE,CTRL,USNP,PHE,varargin)
...
CACOS = cellfun(@vertcat,CASE,CTRL,'UniformOutput',false);
...
for nn = 1:size(CASE,1)
    ...
    if any(CACO)
        ...
        [~,Aj] = ismember(CACOID , SRR );
        Af = Aj(Aj>0);

        vMX(Aj>0,nn) = (HH(Af)+1); %HETALT:2  HOMALT:3
...
PVMX(: , 2)  =  PHE.SET;        % COL2: SET
PVMX(: , 3)  =  PHE.COHORTNUM;  % COL3: COHORT
PVMX(: , 4)  =  PHE.AD;         % COL5: AD
...
PVL(1,:) = abs(PHE.SET)==1;              % In col-1 set CASEs index 1's
PVL(2,:) = abs(PHE.SET)==2;              % In col-2 set CTRLs index 1's
