...
genosdir = fileparts(which('GENOS_NEURALNET_PREPRO.m'));
cd(genosdir);
...
clc; clearvars -except ADSP LOCI CASE CTRL PHEN
disp(LOCI(1:9,:))
...
%% FLIP REF AND ALT FOR VARIANT LOCI WHERE ALT >> REF
...
[TRAINX,TRAINL,TESTX,TESTL] = traintestmx(TRNN, TENN);
...
% INSTEAD OF VARIANTS BEING {0,1,2} MAKE THEM {-2,2,4}
% TRAINMX = ((TRAINMX.*4)-2);
% TESTMX  = ((TESTMX.*4)-2);


% MULTIPLY BY TRAINING P
% TRAINMX = TRAINMX .* (-log(AMX.TRFISHP)); 
% TESTMX  = TESTMX  .* (-log(AMX.TRFISHP));


% SQUARE INPUT VALUES
% TRAINMX = [TRAINMX ; TRAINMX.^2]; 
% TESTMX  = [TESTMX  ; TESTMX.^2]; 
...
writetable(AMX,'AMX.csv')

writetable(PHEN,'PHEN.csv')

writetable(array2table(TRAINMX),'TRAINMX.csv')

writetable(array2table(TRAINLAB),'TRAINLAB.csv')

writetable(array2table(TESTMX),'TESTMX.csv')

writetable(array2table(TESTLAB),'TESTLAB.csv')
