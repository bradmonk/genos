function [varargout] = multinetstats(...
                        net,TRAINMX,TRAINLAB,TESTMX,TESTLAB,...
                        Pfilter,HICI,LOCI,doROC,...
...
h2 = axes('Position',[.53 .62 .40 .33],'Color','none'); 
...
axes(h1); 
ph1=plot((cell2mat((roc(TRL,agenet(TRX)))'))','LineWidth',4);   axis tight
line([0 size(TRL,2)],[0 1],'Color','k','LineStyle','--')
title('Age-Based Training Performance')

axes(h2); 
ph2=plot((cell2mat((roc(TEL,agenet(TEX)))'))','LineWidth',4);   axis tight
line([0 size(TEL,2)],[0 1],'Color','k','LineStyle','--')
title('Age-Based Test Performance')

legend(ph2,{'60s','70s','80s','90s'},'FontSize',12,'Box','off','Location','NorthWest');
...
if size(TPR{1},2) == size(TPR{2},2)
    ...
    axes(h5)
    ph3 = plot(fliplr(FPR'),fliplr(TPR'),'LineWidth',5);
    ph4 = line([0 1],[0 1],'Color','k','LineStyle','--','LineWidth',1);
    box on; grid on
...
legend(ph3,{'CTRL','CASE'},'FontSize',12,'Box','off','Location','NorthWest');
title('Case-Control Training Performance')
