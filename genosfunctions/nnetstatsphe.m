function [varargout] = nnetstatsphe(net,TRAINMX,TRAINLAB,TESTMX,TESTLAB,Pfilter,HICI,LOCI,doROC,PHE)
...
h1 = axes('Position',[.06 .80 .90 .18],'Color','none');
    h1.XLim = [-.5 .5]; h1.XLimMode = 'manual'; colormap(h1,lines(2)); hold on;
h2 = axes('Position',[.06 .62 .90 .12],'Color','none');
...
ph1 = scatter( (rand(size(TRWEIGHT,2),1)-.5).*.1 ,...
            (1:size(TRWEIGHT,2)), 500 , TRLAB, '.');

% axes(h1); colormap(jet)
% ph1 = scatter( TRWEIGHT -.5 ,...
%             (1:size(TRWEIGHT,2)), 500 , PHE(:,3), '.');



axes(h2)
ph2 = scatter( (rand(size(TEWEIGHT,2),1)-.5).*.1 ,...
            (1:size(TEWEIGHT,2)), 500 , TELAB, '.');
